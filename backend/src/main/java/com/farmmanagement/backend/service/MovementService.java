...
public class MovementService {
    ...
    // further (more future edges satisfy ">= an earlier date" than a later one).
    //
    // species is optional: some diseases only spread through a specific
    // species, so a null/blank species traces every movement, while a given
    // species only follows edges carrying that species.
    public List<FarmDistanceResponse> findFarmsWithinMovements(Long farmId, int hops, String species) {
